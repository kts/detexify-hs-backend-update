...
--
-- I was getting an error about 'liftIO' here. I'm not using 'snapshot' stuff, so just comment-out for now:
--
...
loadSuccess c = load c >> return True `Control.Exception.catch` handle where
